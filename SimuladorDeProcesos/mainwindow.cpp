...
void MainWindow::generateProcesses()
{
    ...
    std::uniform_int_distribution<int> distribution_size(5,25);
        ...
        while (tT < readyProcesses[0].getTiempoEstimado() and tT != ACTION_CODE){
            ...
            if (newProcesses.size()){
                if (newProcesses[0].getFrames() <= availableFrames()){
                    ...
                    readyProcesses.push_back(p);
                    newProcessesCount--;
                    if (newProcessesCount){
                        ui->nextProcessLB->setText("Siguiente: ID - " +
                            QString::number(newProcesses[0].getId()) +
                            ", Tamaño: " + QString::number(newProcesses[0].getTamanio()));
                    }else{
                        ui->nextProcessLB->setText("Ningún proceso nuevo.");
                    }
                }
            }
...
void MainWindow::updateMemoryTable()
{
    ...
    for (i = 0; i < FRAMES - 3; i++,rows++){
        ...
        case States::Suspendido:    state = "Suspendido";   break;
        default:
            state = "Libre";
        }
        ui->memoryTB->setItem(rows,3,new QTableWidgetItem(QString(state.c_str())));
    }
    //Operative system frames
    for (i = 42; i < FRAMES; i++){
...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    else if (event->key() == Qt::Key_A and state == RUNNING){
        qDebug() << "A";
        state = PAUSED;
        pause.exec();
    }
    else if (event->key() == Qt::Key_S and state == RUNNING){
        qDebug() << "Suspend";
        suspendProcess();
    }
    else if (event->key() == Qt::Key_R and state == RUNNING){
        qDebug() << "Return";
        returnProcess();
    }
    else{
